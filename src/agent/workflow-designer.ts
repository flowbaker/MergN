...
const PLAN_SYSTEM = [
  ...
  "For a 'webhook' trigger, the steps receive the RAW request body the service POSTs as the trigger input. Describe each step's intent in terms of the provider's REAL payload shape, not invented flat fields: a Stripe webhook's invoice fields live under data.object (input.data.object.number, .amount_paid, .customer_email, .currency) with the event type in input.type; GitHub puts fields at the top level. Say the actual paths in the intent so the step reads the right data.",
...
const BODY_SYSTEM = [
  ...
  "WEBHOOK triggers: when the trigger is a webhook, `input` IS the RAW request body the external service POSTs — read its ACTUAL shape, do NOT invent flat top-level fields. Most providers wrap the data in an envelope. Stripe sends `{ type, data: { object: {...} } }` — read `input.type` for the event and `input.data.object.<field>` for the entity (e.g. an invoice's number is `input.data.object.number`, amount `input.data.object.amount_paid`, customer email `input.data.object.customer_email`, currency `input.data.object.currency`). GitHub sends the event fields at the top level (e.g. `input.action`, `input.pull_request.title`). Match the provider's real payload.",
...
export async function designWorkflow(
  ...
  if (plan.triggerKind === "poll" && plan.poll) {
    ...
    triggerHint =
      "The trigger input is the RAW HTTP request body the external service POSTs. Read its ACTUAL shape — do NOT invent flat top-level field names. Most providers nest the data: a Stripe webhook sends `{ type, data: { object: {...} } }`, so read `input.type` for the event and `input.data.object.<field>` for the entity (e.g. an invoice's number is input.data.object.number, amount input.data.object.amount_paid, customer email input.data.object.customer_email, currency input.data.object.currency). GitHub puts the fields at the top level (input.action, input.pull_request.title). Match the provider implied by the user's goal.";
  }
