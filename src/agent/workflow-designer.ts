...
const PLAN_SYSTEM = [
  ...
  "For a 'webhook' trigger, the step gets the ENTIRE raw request body as `input.payload` (you don't know the exact shape at design time, and it differs per service — never assume one). For 'forward/format/store the webhook data' goals, the step should just process `input.payload` as-is — do NOT invent flat trigger field names. Only when the user names a specific field, navigate the payload along the path THEY describe (e.g. input.payload.<field> or input.payload.<a>.<b>). Keep webhook workflows to a single step that handles input.payload unless the goal truly needs more.",
